...
import type { CompanyChangeRequest, WithdrawalRequest } from "../services/organizations";
import { renderCompanyChangeAlert } from "./templates/company-change-alert";
import { renderWithdrawalAlert } from "./templates/withdrawal-alert";
...
export async function notifyWithdrawalRequested(env: OrganizationNotificationEnv, request: WithdrawalRequest): Promise<void> {
  const to = env.MAIL_ADMIN_ALERTS;
  const adminUrl = env.ADMIN_URL;
  if (!to || !adminUrl) {
    logIntegrationError({ requestId: newRequestId(), service: "resend", action: "withdrawal_alert" }, "MAIL_ADMIN_ALERTS or ADMIN_URL is not set; operator alert not sent");
    return;
  }

  await sendMail(env, (context) =>
    renderWithdrawalAlert(
      {
        to,
        organizationName: request.organizationName,
        orgCode: request.orgCode,
