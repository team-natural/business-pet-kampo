...
| **`Exit code: 137` / `Page crashed` / dev サーバーが途中で落ちる** | コンテナのメモリ切れ。長いセッションでは `workerd` が孤児として残り 1 プロセス 300〜400MB を保持し続ける。`! ps -eo pid,stat,comm \| awk '$3=="workerd" && $2 !~ /^Z/ {print $1}' \| xargs -r kill -9` で回収してから再実行する |
