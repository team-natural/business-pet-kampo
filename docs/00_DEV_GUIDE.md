...
| **5. 機能実装** | 実装ステージ S1〜S16 を順に入れる（1 ステージ = 1 ブランチ = 1 PR） | 会話で依頼（着手順の出典は **§3-3a**。優先度列ではない — GOV-01 D-032） | — |
...
「S3（新規取引申請の送信）を実装して」
  → CLAUDE.md のレイヤー原則（Astro Page/API Route → Service → D1、DEV-01 §5 参照）に従い実装
    認可は Service 層の入口で強制する。`requireAdminUser`（admin 側）/
    `requireActiveOrganization`（public 側）で、ロールによる分岐は無い（GOV-01 D-014）
...
**着手順は優先度順ではなく、業務ライフサイクル順に決める**（GOV-01 D-032）。PRD-03 の優先度列は
「何が重要か」であって「何から作れるか」ではない。カート（F-04-01、High）は承認（F-02-06、High）が
Member を作るまで動かしようがなく、優先度順に並べると毎回テストデータを手で INSERT することになり、
承認フローの不具合が最後まで表に出ない。依存の背骨は次のとおり：

```
申請送信 → 審査・承認 → Organization + Member 生成 → 有効化・ログイン
        → 卸価格が見える → カート → 発注確定 → 受注対応
```

### 3-3a. 実装ステージ（S1〜S16）

**1 ステージ = 1 ブランチ = 1 PR。** 各ステージは終了時に「手で触って確認できる状態」になるよう
区切ってある。ブランチ名は `feat/<slug>`（基盤整備は `chore/<slug>`）で、`dev` に向けて PR を出す。

> **「フェーズ」と呼ばない。** BIZ-01 / BIZ-02 の「フェーズ」はリリース後の事業フェーズ（KPI 目標の
> 区切り）を指す既存の語であり、実装の区切りと混同すると BIZ-02 §3 の読み違いが起きる。
>
> 本節は着手順と依存関係のみを定義する。日程・工数・担当の割り付けは文書化しない（00_README §0）。
...
| S8 | `feat/cart` | カート追加/変更/削除・単価解決・金額計算（SCR-21）。**配送方法・配送地域・分割配送は含めない**（TBD-07 未決。送料は D-013 の全国一律のみ） | PRD-03 FG-04、BIZ-03 §3-1 | S5、**TBD-07**（配送部分のみ） | 金額が `commerce.ts` と一致／存在しない slug が 400／発注単位違反を拒否 |
| S9 | `feat/checkout-bank-transfer` | 発注確定（銀行振込のみ）・スナップショット・発注履歴（SCR-18・19・22〜24）。**マイページトップ（SCR-12）の発注状況サマリもここで入れる** — S7 では `orders` に行が無く、0 件表示は「注文が無い」と読めてしまうため意図的に出していない。**金額は `getCart()` が返す `totals` をそのまま使う** — checkout 側で `orderTotals` を呼び直すとカート画面と発注金額が食い違う（S8）。配送は **1 注文 1 配送先**（D-038）、振込は **7 営業日・取引先負担・手動消込・振込依頼人名に注文番号**（D-039）。`orders.payment_due_at` の列追加が要る（D-040） | DEV-05 §3、DEV-07 §6-0、GOV-01 D-038〜D-041 | S7, S8 | 発注後に Markdown の価格を変えても過去注文の金額が動かない |
| S10 | `feat/order-admin` | 受注管理・遷移 4 ルート・入金確認（**手動消込** — D-039）・キャンセル（ADM-17・18）。**残りの配送項目（配送会社・方法・地域・日時指定・追跡番号）の `orders` 列追加もここで**（TBD-07）。振込期限超過の扱いは TBD-04c | DEV-09 §2-5・§2-6、DEV-04 §5-7 | S9、TBD-07・TBD-04c | DEV-09 §2-5-2 の全セルをテスト／不正遷移が 409 |
...
- S2 は S1・S3〜S7 と並行可（触るファイルがほぼ重ならない）
- S12 は S5 以降いつでも独立して着手できる
- 外部手配（Resend のドメイン認証・決済アカウント・OAuth アプリ登録・Access ポリシー）は
  リードタイムがあるため、S1 の時点で全て並行開始する。実装完了後に着手すると待ちになる

**着手前に解決が必要な P0（GOV-02）**

| TBD | 止まるステージ | 理由 |
| --- | --- | --- |
| ~~**TBD-07**~~（分割配送の可否）| — | **解決済み（D-038: 分割配送なし）**。スキーマ変更は不要になった。残る配送項目（配送会社・方法・地域・日時指定・追跡番号）は P2 に降格し S10 で列追加として扱う。旧記述: 分割配送を採ると `orders` 1 : n の出荷テーブルが要り、**スキーマ変更を伴う**（DEV-07 §6-2 は 1 注文 1 配送先のスナップショットのみ）|
| ~~**TBD-04b**~~（銀行振込の口座・期限・消込方法）| — | **解決済み（D-039・D-040）**。7 営業日・取引先負担・手動消込・振込依頼人名に注文番号。口座の実値だけ本番デプロイ前に要入力（env、未設定なら口座欄を出さない）|
| ~~**TBD-34**~~（注文番号の採番方式）| — | **解決済み（D-041: `YYYYMMDD-NNN`）**|
| TBD-04c（振込期限超過の扱い）| S10 | S9 は期限を表示するだけで閉じられる |
| TBD-04d（営業日に祝日を含めるか）| S16 | 実装は土日のみ除外。期限は発注時に保存されるため案内と画面は一致する |
| **口座の実値**（`BANK_*`）| S16 | 未設定なら口座欄を出さない作りになっているため実装は止まらないが、**本番リリース前に必須** |
...
| **`Exit code: 137` / `Page crashed` / dev サーバーが途中で落ちる** | コンテナのメモリ切れ。長いセッションでは `workerd` が孤児として残り 1 プロセス 300〜400MB を保持し続ける。`! ps -eo pid,stat,comm \| awk '$3=="workerd" && $2 !~ /^Z/ {print $1}' \| xargs -r kill -9` で回収してから再実行する。**回収しても足りない場合はコンテナを再起動する** — MCP サーバー等が数百 MB ずつ保持しており、セッションが長いほど余地が減る |
