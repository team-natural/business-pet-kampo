...
| 無効化したボタン | **無効に見えること**（`global.css` の `.btn:disabled`）と、**無効な理由が読み上げられること**。本サイトのフォームは未ハイドレート時・送信中・入力が規則に反する時に submit を無効化するため、規則は `aria-describedby` で入力欄に結び付ける（例: 発注単位の倍数。`add-to-cart.svelte`）。理由の無い無効化は「押したのに何も起きない」と同じ |
| カラーコントラスト | WCAG AA 以上。**半透明で色を薄めない** — `text-ink-soft/70` のようなプレースホルダーは紙色の上で 3:1 前後まで落ちる（S2 で修正済み）|
