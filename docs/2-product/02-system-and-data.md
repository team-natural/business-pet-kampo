...
| SEO | **`/sitemap.xml` を動的ルートとして自前生成する**（`apps/public/src/pages/sitemap.xml.ts`）。`@astrojs/sitemap` は採用しない — 同 integration は公式に「SSR モードの動的ルートのサイトマップエントリを生成できない」とされており、D-021 により商品・お知らせページは prerender できないため、**サイトマップに載せるべきページがちょうど全部載らない**（GOV-01 D-037）。生成元は `lib/catalog.ts` の `sitemapPaths()` 1 か所。**会員ルート（`/mypage/**`）・取引系ルート（`/cart`・`/checkout/**`）・申請/有効化/パスワードのトークン付きルート・管理サブドメイン・`draft` および `client_only` のお知らせを除外する** — 除外リストではなく `STATIC_SITEMAP_PATHS` への allow-list で表現する（除外リストは追加を忘れる）。オリジンは `APP_URL` から取り、`astro.config` の `site` に依存しない（TBD-16 待ちにしない）。noindex を返すページをサイトマップに載せない。`robots.txt` も同じ理由で動的ルート。**`lastmod` / `changefreq` / `priority` は出力しない** — Content Collections に信頼できる更新日時が無く（お知らせの `publishedDate` は公開日であって更新日ではなく、商品には日付項目自体が無い）、嘘の `lastmod` はクローラーに無視されるか、悪ければ再クロールを遅らせるため。更新日を frontmatter に持たせる判断をしたら、そのとき併せて出力する |
